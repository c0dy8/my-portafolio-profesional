...
export const MagicTextReveal: React.FC<MagicTextRevealProps> = ({
  ...
  useEffect(() => {
    const handleResize = () => {
      if (wrapperRef.current && textDimensions.width && textDimensions.height) {
        ...
        const basePadding = isMobile ? Math.max(fontSize * 0.15, 12) : Math.max(fontSize * 0.22, 20);
        ...
        setWrapperSize({ width: fillWidth ? maxWidth : Math.min(minWidth, maxWidth), height: Math.min(minHeight, maxHeight) });
      }
    };
    if (textDimensions.width && textDimensions.height) handleResize();
    window.addEventListener('resize', handleResize);
    return () => window.removeEventListener('resize', handleResize);
  }, [textDimensions, fontSize, fillWidth]);
  ...
  return (
    ...
    >
      <div
        className={`absolute z-10 transition-opacity duration-200 select-none ${showText ? 'opacity-100' : 'opacity-0'}`}
        style={{
          color,
          fontFamily,
          fontWeight,
